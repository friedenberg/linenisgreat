...
- **Nix flake support requires _smart_ HTTP** — Nix's `git+https` fetcher does ref
  ...
  `ref`/`rev` resolution.
- **`go get` is _not_ a hard constraint** — Go consumers of these modules resolve
...
- **Dodder pivot** — `build-code-github` (`justfile:74-129`) is explicitly a
  _"TEMPORARY GitHub bridge: dodder is meant to re-become the source of truth …
  at which point the API itself is intended to be served from a dodder repo."_
...
The decision has **two axes**: (A) _where_ the git endpoint runs, and (B) _what
backs it_ (proxy GitHub vs. mirror vs. dodder). Hosting is the binding constraint,
so options below are framed by host.

> **Key clarification — "dumb proxy" vs. "dumb protocol".** GitHub already speaks
> the git _smart_ HTTP protocol. A **transparent ("dumb") reverse proxy** that just
> forwards bytes therefore _carries_ the smart protocol end-to-end — it does **not**
...
- Bad: operationally heavier — a process to keep alive, restart, and monitor on a
  host chosen precisely for _not_ having long-running processes.
...
- Note: because `go get` is not a constraint (see Decision Drivers —
  `flake-input-go_mod`), the host can serve _only_ git smart HTTP. We can CNAME the
...
- It satisfies the Nix-flake driver: a transparent proxy carries GitHub's _smart_
...
- It stays inside the **plain-PHP + .htaccess** model — **no Server Type change** —
  preserving ADR-0001; the change is one router rule + one script, replacing the 302.
- It keeps GitHub as the _current_ origin, matching the "temporary GitHub bridge"
...
- The one real risk — NFSN's **3-minute wall-clock PHP cap** — is immaterial for
  ...
  CNAME), which serves the _same_ URL without the cap.
...
| **1 — NFSN resident daemon**         | **~$1–5/mo**       | NFSN bills resources per RAU (1 GiB·min ≈ memory·time). A small always-on daemon (~128–256 MB) bills continuously regardless of traffic; storage is $1.00/GiB-month; bandwidth $0.10/GiB if/when charged. ([storage FAQ][nfsn-storage]) A reference data point: four _static_ NFSN sites ≈ $3.92/mo. |
...
- Gotchas: `ref` is assumed to be a branch (`refs/heads/`), so tags need `?tag=`;
  supplying `ref` _and_ `rev` together is rejected by Nix.
